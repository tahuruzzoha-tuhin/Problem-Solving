...
ll get_sum(ll a, ll b, ll c, ll d){
    c = min<ll>(c, n);
    d = min<ll>(d, m);
...
void Accepted()
{
    ...
    for (ll i = 1; i<=n; i++){
        for (ll j = 1; j <= m; j++){
            prefix_sum[i][j] = prefix_sum[i - 1][j] + prefix_sum[i][j - 1] - (prefix_sum[i - 1][j - 1]) + (ll)(grid[i][j] == '1');
        }
    }
    ...
    for (ll i = 2; i<=n || i<=m; i++){
        ...
        for (ll j = 1; j<=n; j+=i) {
            for (ll k = 1; k<= m; k+=i) {
                ll subsquare = get_sum(j, k, j + i - 1, k + i - 1);
                sum += min<ll>(subsquare, i * i - subsquare);
            }
        }
        result = min<ll>(result, sum);
    }
    printf("%lld", result);
