...
void Accepted()
{
    ...
    ll subtable1[2] = {n-1, n*(m-1)};
    ll subtable2[2] = {m-1, m*(n-1)};

    sort(subtable1, subtable1+2);
    sort(subtable2, subtable2+2);

    ll max1 = arr.back()-arr.front();
    ll max2 = max(arr[arr.size()-2]-arr.front(), arr.back()-arr[1]);

    ll ans = subtable1[0]*max2 + subtable1[1]*max1;
    ans = max(ans, subtable2[0]*max2 + subtable2[1]*max1);
