...
// wstring read_string_long() {
//     wstring s; wchar_t wc;
//     while (std::iswspace(wc = std::getwchar()));
//     if (wc != std::WEOF && !std::iswcntrl(wc)) {
//         s.push_back(wc);
//     }
//     while ((wc = std::getwchar()) != std::WEOF && !std::iswspace(wc) && !std::iswcntrl(wc)) {
//         s.push_back(wc);
//     }
//     return s;
// }
...
void business_logic()
{
    ll n; string name[1010]; ll score[1010];
    cin >> n;
    map<string, ll> mk, aw;
    f0(i,n){
        cin >> name[i] >> score[i];
    ...
    ll mx = -Inf;
    f0(i,n) mx = max(mx, mk[name[i]]);
    f0(i,n){
        aw[name[i]] += score[i];
        if(aw[name[i]] >= mx && mk[name[i]] >= mx ){
            cout << name[i] << endl;
            break;
        }
    }
