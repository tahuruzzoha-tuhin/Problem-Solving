...
int main() {
    ...
    while (t--) {
        ...
        if (n != m) { // if lengths don't match, not possible
            possible = false;
        } else {
            for (int i = 0; i < n; i++) {
                if (a[i] == b[i]) { // if both characters match, add to template
                    template_str += a[i];
                } else { // if characters don't match, add an asterisk to template
                    template_str += "*";
                }
            }
            
            int num_asterisks = 0, num_letters = 0;
            ...
            if (num_asterisks > num_letters) { // if more asterisks than letters, not possible
                possible = false;
            }
        }
