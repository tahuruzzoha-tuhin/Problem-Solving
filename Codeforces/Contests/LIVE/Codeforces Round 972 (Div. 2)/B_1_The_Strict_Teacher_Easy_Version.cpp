...
int32_t main()
{
    ...
    // while(Test_kase--) bvvusiness_logic();
    // TLE;
    return 0;
}
...
void business_logic()
{
    ll n=0, m=0, q=0, teacher1=0, teacher2=0, at=0;
    cin >> n >> m >> q ;
    cin >> teacher1 >> teacher2 ;
    cin >> at;

    if(at < min(teacher1,teacher2)){
        cout << min(teacher1,teacher2)-1 << endl;
    } else if (at > max(teacher1,teacher2)){
        cout << n-max(teacher1,teacher2) << endl;
    } else {
        int radix = (teacher1+teacher2)/2;
        cout << min(abs(radix-teacher1), abs(radix-teacher2)) << endl;
    }
