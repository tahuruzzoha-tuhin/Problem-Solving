...
void business_logic()
{
    ...
    while(q--){
        ...
        } else if(at > teachers.back()){
            cout << n-teachers.back() << endl;
        } else {
            auto optimal = upper_bound(teachers.begin(), teachers.end(), at);
            auto moves = optimal;
            moves--;

            auto radix = (*optimal + *moves) / 2;
            auto opt_mov1 = abs(radix-*optimal);
            auto opt_mov2 = abs(radix-*moves);
            cout << min(opt_mov1, opt_mov2) << endl;
        }
    }
